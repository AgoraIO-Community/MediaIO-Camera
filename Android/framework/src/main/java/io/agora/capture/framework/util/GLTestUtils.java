...
public class GLTestUtils {
    ...
    private static Bitmap rgba8888ToBitmap(byte[] rgba8888, int width, int height) {
        Bitmap bitmap = null;
        try {
            bitmap = Bitmap.createBitmap(width, height, Bitmap.Config.ARGB_8888);
            ByteBuffer rgbaBuf = ByteBuffer.allocateDirect(width* height * 4);
            rgbaBuf.put(rgba8888).position(0);
            bitmap.copyPixelsFromBuffer(rgbaBuf);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private static Bitmap rgb565ToBitmap(byte[] rgb565, int width, int height) {
        Bitmap bitmap = null;
        try {
            bitmap = Bitmap.createBitmap(width, height, Bitmap.Config.RGB_565);
            ByteBuffer rgbaBuf = ByteBuffer.allocateDirect(width* height * 2);
            rgbaBuf.put(rgb565).position(0);
            bitmap.copyPixelsFromBuffer(rgbaBuf);
        } catch (Exception e) {
            e.printStackTrace();
        }
